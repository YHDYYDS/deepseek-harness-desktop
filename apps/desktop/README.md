...
- 🔄 **应用内自动更新（安装版）**：启动后自动静默检查 + 帮助 → 检查更新，发现新版后台下载、一键重启安装；便携版因无法替换自身，自动降级为「打开 Releases 页」
...
5. 更新：按 `Alt` 呼出菜单 → 帮助 → 检查更新；启动 25 秒后也会自动静默检查，发现新版下载完成会弹窗提示一键重启安装。
...
- **自动更新仅限安装版**（setup.exe）：便携版运行时无法替换自身，请从 Releases 下载新包覆盖。
